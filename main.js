...
async function initVisualizer() {
  ...
  const spectroContainer = document.querySelector("#spectroVisualizer");
  const visualizerDock = document.querySelector(".visualizer-dock");
  const expandButton = document.querySelector("#expandVisualizer");
  const pauseButton = document.querySelector("#pauseDemo");
  const micButton = document.querySelector("#micButton");
  const demoButton = document.querySelector("#demoButton");

  const spectroRenderer = new THREE.WebGLRenderer({ antialias: true, alpha: true });
  spectroRenderer.setPixelRatio(Math.min(window.devicePixelRatio, 2));
  spectroContainer.appendChild(spectroRenderer.domElement);

  const {
    width: initialSpectroWidth,
    height: initialSpectroHeight
  } = spectroContainer.getBoundingClientRect();

  const spectroScene = new THREE.Scene();
  const spectroCamera = new THREE.OrthographicCamera(-1, 1, 1, -1, 0.1, 10);
  spectroCamera.position.z = 2;

  const spectroLineCount = 128;
  const spectroPositions = new Float32Array(spectroLineCount * 2 * 3);
  const spectroColors = new Float32Array(spectroLineCount * 2 * 3);
  const spectroGeometry = new THREE.BufferGeometry();
  const spectroMaterial = new THREE.LineBasicMaterial({
    vertexColors: true,
    transparent: true,
    opacity: 0.78,
    blending: THREE.AdditiveBlending
  });
  const spectroLines = new THREE.LineSegments(spectroGeometry, spectroMaterial);
  spectroGeometry.setAttribute("position", new THREE.BufferAttribute(spectroPositions, 3));
  spectroGeometry.setAttribute("color", new THREE.BufferAttribute(spectroColors, 3));
  spectroScene.add(spectroLines);
  ...
  function setDemoControlState(isPlaying, label = "Play Demo") {
    demoButton.textContent = isPlaying ? "Pause Demo" : label;
    pauseButton.textContent = isPlaying ? "||" : ">";
    pauseButton.setAttribute(
      "aria-label",
      isPlaying ? "Pause demo track" : "Play demo track"
    );
    pauseButton.setAttribute(
      "title",
      isPlaying ? "Pause demo track" : "Play demo track"
  ...
  async function playDemo() {
    ...
    if (!demoAudio) {
      demoAudio = new Audio("assets/TechnoBeat.mp3");
      demoAudio.loop = true;
      demoSource = context.createMediaElementSource(demoAudio);
  ...
  function demoValue(index, time) {
    if (isAudioActive) {
      const wave = Math.sin(time * 0.004 + index * 0.34);
      const pulse = Math.sin(time * 0.0016 + index * 0.09);
      return 0.12 + Math.max(0, wave * 0.72 + pulse * 0.24);
    }

    return 0.015 + Math.sin(time * 0.0008 + index * 0.11) * 0.006;
  }
  ...
  function updateSpectrograph(time) {
    for (let index = 0; index < spectroLineCount; index += 1) {
      const positionIndex = index * 6;
      const colorIndex = index * 6;
      const bin = Math.floor((index / spectroLineCount) * 112);
      const audioLevel = frequencyData ? frequencyData[bin] / 255 : demoValue(index, time);
      const flicker = Math.sin(time * 0.009 + index * 1.7) * 0.045;
      const x = THREE.MathUtils.mapLinear(index, 0, spectroLineCount - 1, -0.96, 0.96);
      const centerY = Math.sin(time * 0.0012 + index * 0.08) * 0.025;
      const halfHeight = THREE.MathUtils.clamp(0.035 + audioLevel * 0.68 + flicker, 0.025, 0.78);
      const isPink = index % 17 === 0 || audioLevel > 0.9;

      spectroPositions[positionIndex] = x;
      spectroPositions[positionIndex + 1] = centerY - halfHeight;
      spectroPositions[positionIndex + 2] = 0;
      spectroPositions[positionIndex + 3] = x;
      spectroPositions[positionIndex + 4] = centerY + halfHeight;
      spectroPositions[positionIndex + 5] = 0;

      const red = isPink ? 1 : 0;
      const green = isPink ? 0 : 0;
      const blue = isPink ? 1 : 1;

      spectroColors[colorIndex] = red * 0.7;
      spectroColors[colorIndex + 1] = green * 0.7;
      spectroColors[colorIndex + 2] = blue * 0.7;
      spectroColors[colorIndex + 3] = red * 0.7;
      spectroColors[colorIndex + 4] = green * 0.7;
      spectroColors[colorIndex + 5] = blue * 0.7;
    }

    spectroGeometry.attributes.position.needsUpdate = true;
    spectroGeometry.attributes.color.needsUpdate = true;
  }

  function resize() {
    const { width: spectroWidth, height: spectroHeight } = spectroContainer.getBoundingClientRect();

    spectroCamera.left = -1;
    spectroCamera.right = 1;
    spectroCamera.top = 1;
    spectroCamera.bottom = -1;
    spectroCamera.updateProjectionMatrix();
    spectroRenderer.setSize(spectroWidth || initialSpectroWidth, spectroHeight || initialSpectroHeight, false);
  }

  function animate(time) {
    const elapsed = time - startedAt;
    readFrequencyData();
    updateSpectrograph(elapsed);
  ...
  expandButton.addEventListener("click", () => {
    const isExpanded = visualizerDock.classList.toggle("is-expanded");
    expandButton.setAttribute("aria-expanded", String(isExpanded));
    expandButton.setAttribute(
      "aria-label",
      isExpanded ? "Collapse visualizer" : "Expand visualizer"
    );
    expandButton.textContent = isExpanded ? "-" : "+";
    resize();
  });

  pauseButton.addEventListener("click", toggleDemo);

  const panelObserver = new ResizeObserver(resize);
  panelObserver.observe(spectroContainer);
